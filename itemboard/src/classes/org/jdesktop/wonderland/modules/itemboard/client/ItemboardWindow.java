...
import org.jdesktop.wonderland.modules.itemboard.client.cell.ItemboardCell;
...
public class ItemboardWindow extends Window2D
{
  ...
  private static final Logger LOGGER
    = Logger.getLogger(ItemboardWindow.class.getName());
  private static final ResourceBundle BUNDLE = ResourceBundle.getBundle(
    "org/jdesktop/wonderland/modules/itemboard/client/resources/Bundle");
  private ItemboardDrawingSurface wbSurface;
  private ItemboardCell cell;
  private ItemboardToolManager toolManager;
  private ItemboardDocument itemboardDocument;
  private ItemboardComponent commComponent;
  private AffineTransform scaleTransform;
  private float zoom = 1.0f;
  private boolean synced = true;
  private ItemboardControlPanel controls;
  ...
  private ItemboardMouseListener svgMouseListener;
  private ItemboardSelection selection = null;
  ...
  private ItemboardGVTTreeRendererListener itemboardGVTTreeRendererListener;
  private ItemboardUpdateManagerListener itemboardUpdateManagerListener;
   ...
   * @param topLevel Whether the window is top-level (e.g. is decorated) with a
   * frame.
   * @param pixelScale The size of the window pixels.
   * @param commComponent The communications component for communicating with
   * the server.
   */
  public ItemboardWindow(ItemboardCell cell, App2D app, int width, int height, boolean topLevel, Vector2f pixelScale,
    final ItemboardComponent commComponent)
    throws InstantiationException
  {
    super(app, Type.PRIMARY, width, height, topLevel, pixelScale, new ItemboardDrawingSurface(width, height));
    LOGGER.info("creating itemboard with size: " + width + "x" + height);
    this.cell = cell;
    this.commComponent = commComponent;
    setTitle(BUNDLE.getString("Itemboard"));
    initCanvas(width, height);
    initHUD();
    setDisplayMode(DisplayMode.HUD);
    showControls(false);
    wbSurface = (ItemboardDrawingSurface) getSurface();

    itemboardDocument = new ItemboardDocument(this);
    addEventListeners();
  }
  ...
  private void initCanvas(int width, int height)
  {
    ...
    svgCanvas.addSVGDocumentLoaderListener(itemboardDocument);
    itemboardGVTTreeRendererListener = new ItemboardGVTTreeRendererListener(this);
    svgCanvas.addGVTTreeRendererListener(itemboardGVTTreeRendererListener);
    itemboardUpdateManagerListener = new ItemboardUpdateManagerListener((ItemboardApp) this.getApp());
    svgCanvas.addUpdateManagerListener(itemboardUpdateManagerListener);
  }
  ...
  {
    svgMouseListener = new ItemboardMouseListener(this, itemboardDocument);
    addMouseWheelListener(svgMouseListener);
    addMouseMotionListener(svgMouseListener);
    addMouseListener(svgMouseListener);
    addKeyListener(new ItemboardKeyListener(this));
  }
  ...
  private void removeEventListeners()
  {
    ...
    removeKeyListener(new ItemboardKeyListener(this));
  }
  ...
  public void cleanup()
  {
    ...
    svgCanvas.removeSVGDocumentLoaderListener(itemboardDocument);
    svgCanvas.removeGVTTreeRendererListener(itemboardGVTTreeRendererListener);
    svgCanvas.removeUpdateManagerListener(itemboardUpdateManagerListener);
    svgCanvas.dispose();
    ...
    EventQueue.invokeLater(new Runnable()
    {
      ...
      public void run()
      {
        ...
        if (controlComponent == null)
        {
          ...
          controls = new ItemboardControlPanel(ItemboardWindow.this);

          // add event listeners
          toolManager = new ItemboardToolManager(ItemboardWindow.this);
  ...
  protected class DrawingOverlay implements Overlay
  {
    ...
    public void paint(Graphics g)
    {
      ...
      if (currentPoint != null)
      {
        ...
        ItemboardTool currentTool = toolManager.getTool();
        if (currentTool == ItemboardTool.LINE)
        {
        ...
        {
          Rectangle r = ItemboardUtils.constructRectObject(pressedPoint, currentPoint);
          if (currentTool == ItemboardTool.RECT)
          {
            LOGGER.fine("drawing rectangle: " + r);
            g2d.draw(r);
          }
          else if (currentTool == ItemboardTool.ELLIPSE)
          {
   ...
   *
   * @param immediately if true, remove the message now, otherwise slide it off
   * the screen first
   */
  ...
  public void singleSelection(Point p)
  {
    ...
    for (int i = 0; i < childNodes.getLength(); i++)
    {
      ...
      if (aNode != null && aNode instanceof Element)
      {
        ...
        {
          selection = new ItemboardSelection((Element) aNode, aGraphicsNode.getBounds());
          selectionMade = true;
        }
      }
    }
    ...
    String docURI, Point position, Float zoom)
  {
    ...
    ItemboardCellMessage msg = new ItemboardCellMessage(getClientID(app), getCellID(app),
...
//        logger.finest("itemboard: paint");
  ...
  {
    itemboardDocument.setSVGDocument(document);
  ...
  {
    return itemboardDocument.getSVGDocument();
  }
  ...
  {
    itemboardDocument.openDocument(uri, notify);
  }

  public void newDocument(boolean notify)
  {
    SVGDocument document = (SVGDocument) ItemboardUtils.newDocument();
    svgCanvas.setDocument(document);
    itemboardDocument.setSVGDocument(document);
  ...
  {
    itemboardDocument.showSVGDialog();
  }
  ...
  public ItemboardTool getCurrentTool()
  {
    return toolManager.getTool();
  }

  public ItemboardSelection getSelection()
  {
    return selection;
  }
   ...
   */
  public ItemboardToolManager getToolManager()
  {
    return toolManager;
  }
   ...
   */
  public void setSelection(ItemboardSelection selection)
  {
  ...
  public void addNewElement(final Element e, boolean notify)
  {
      ...
      invokeLater(new Runnable()
        {
          ...
          public void run()
          {
            ...
            itemboardDocument.appendChild(e);
          }
      });
    ...
    {
      // notify
      sendRequest(Action.ADD_ELEMENT, ItemboardUtils.elementToXMLString(e), null, null, null);
    }
  }
  ...
  {
    return itemboardDocument.moveElement(toMove);
  }
  ...
  {
    return itemboardDocument.moveElement(toMove, xDiff, yDiff);
  }
  ...
  {
    return (Element) itemboardDocument.importNode(e, true);
  }
  ...
  public void removeElement(final Element toRemove, boolean notify)
  {
    svgCanvas.getUpdateManager().getUpdateRunnableQueue().invokeLater(new Runnable()
    {
      ...
      public void run()
      {
        ...
        Element rem = itemboardDocument.getElementById(toRemove.getAttributeNS(null, "id"));
        if (rem != null)
        {//In case this client has already removed it
          itemboardDocument.removeChild(rem);
        }
    ...
    {
      // notify
      sendRequest(Action.REMOVE_ELEMENT, ItemboardUtils.elementToXMLString(toRemove), null, null, null);
    }
  }
  ...
  public void updateElement(final Element afterMove, boolean notify)
  {
    svgCanvas.getUpdateManager().getUpdateRunnableQueue().invokeLater(new Runnable()
    {
      ...
      public void run()
      {
        ...
        itemboardDocument.replaceChild(afterMove, itemboardDocument.getElementById(afterMove.getAttributeNS(null, "id")));
      }
    });
    ...
    {
      // notify
      sendRequest(Action.UPDATE_ELEMENT, ItemboardUtils.elementToXMLString(afterMove), null, null, null);
    }
  }
}
